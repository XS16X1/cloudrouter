...
// 请求计数相关
const REQUEST_COUNT_KEY_PREFIX = 'req_count_';
...
// 导出 setter 函数
function setApiKeys(newApiKeys) {
  apiKeys = newApiKeys;
}

function setCurrentKeyIndex(index) {
  currentKeyIndex = index;
}

function setLastHealthCheck(timestamp) {
...
// 添加新的 API 密钥
async function addApiKey(name, value, env) {
...
// 删除 API 密钥
async function deleteApiKey(name, env) {
  await initializeState(env);
  const currentKeys = getApiKeys();
...
// 刷新所有密钥健康状态
async function refreshAllKeyHealth(env) {
  await initializeState(env);
...
// 生成密钥名称（自动生成）
function generateKeyName(index) {
  return `Key-${Date.now()}-${index.toString().padStart(3, '0')}`;
}

// 批量添加 API 密钥
async function batchAddApiKeys(keyValues, env) {
  await initializeState(env);
  const currentKeys = getApiKeys();
...
// 批量删除 API 密钥
async function batchDeleteApiKeys(names, env) {
  await initializeState(env);
  const currentKeys = getApiKeys();
...
// 获取请求计数
async function getRequestCounts(env) {
  await initializeState(env);
  const currentKeys = getApiKeys();
...
// 增加请求计数
async function incrementRequestCount(keyName, env) {
...
// 创建新的客户端 token
async function createToken(name, customToken, env) {
  await initializeState(env);
...
// 更新 token 状态
async function updateTokenStatus(name, enabled, env) {
  await initializeState(env);
  const currentTokens = getClientTokens();
...
// 删除 token
async function deleteToken(name, env) {
  await initializeState(env);
  const currentTokens = getClientTokens();
...
// --- 管理员 API 路由 ---
function setupAdminApiRoutes(router) {
  // 检查管理员密码设置状态
...
// --- OpenAI 兼容 API ---
function setupOpenaiApiRoutes(router) {
  // 获取模型列表
    ...
    <script>
        ...
        function renderApiKeys(keys, counts = {}) {
            ...
            keysList.innerHTML = keys.map(key => {
                ...
                const requestCount = counts[key.name] || 0;
                
                return '<tr>' +
                    '<td><span class="status ' + statusClass + '"></span> ' + statusText + '</td>' +
                    '<td>' + escapedName + '</td>' +
                    '<td style="text-align: center; font-weight: bold;">' + requestCount + '</td>' +
